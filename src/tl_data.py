        ...
        with open(fname, "rb") as handle:
            ...
            for i, (key, value) in enumerate(tl_data.items()):
                ...
                for j in range(nsection):
                    airfoil_2d[i * nsection + j, ...] = np.concatenate(
                        (
                            np.linspace(0.0, 1.0, 200).reshape((-1, 1)),
                            geo_2d[0, :, :2],
                            field_2d[0, :, 0].reshape((-1, 1)),
                        ),
                        axis=1,
                    )
                    geometry_3d[i * nsection + j, ...] = np.concatenate(
                        (np.linspace(0.0, 1.0, 200).reshape((-1, 1)), geo_3d[j, :, :2]),
                        axis=1,
                    )
    ...
    def fit(self, data_loader):
        ...
        stats = {
            ...
            "coord_3d": {
                ...
                "min": float("inf"),
                "max": float("-inf"),
            },  # MinMax/Std for 3D coordinates
            "z_coord": {
                "mean": 0.0,
                "std": 1.0,
                "min": float("inf"),
                "max": float("-inf"),
            },  # MinMax/Std for z-coordinates
            "mach": {
                "mean": 0.0,
                "std": 1.0,
                "min": float("inf"),
        ...
        for batch in data_loader:
            ...
            stats["pressure_3d"]["min"] = min(
                stats["pressure_3d"]["min"], p_3d.min().item()
            )
            stats["pressure_3d"]["max"] = max(
                stats["pressure_3d"]["max"], p_3d.max().item()
            )
            ...
            stats["coord_3d"]["min"] = min(
                stats["coord_3d"]["min"], coords_3d.min().item()
            )
            stats["coord_3d"]["max"] = max(
                stats["coord_3d"]["max"], coords_3d.max().item()
            )
        ...
        stats["pressure_2d"]["mean"] = (
            sum_pressure_2d / (n_samples * 200)
        ).item()  # 200 points for pressure
        stats["pressure_3d"]["mean"] = (
            sum_pressure_3d / (n_samples * 200)
        ).item()  # 200 points for pressure
        stats["coord_2d"]["mean"] = (
            sum_coords_2d / (n_samples * 200)
        ).item()  # 200 points for pressure
        stats["coord_3d"]["mean"] = (
            sum_coords_3d / (n_samples * 200)
        ).item()  # 200 points for pressure
        ...
        stats["pressure_2d"]["std"] = np.sqrt(
            sum_sq_pressure_2d / (n_samples * 200)
        ).item()
        stats["pressure_3d"]["std"] = np.sqrt(
            sum_sq_pressure_3d / (n_samples * 200)
        ).item()
